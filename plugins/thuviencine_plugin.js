...
function parseEmbedResponse(html, sourceUrl) {
    try {
        ...
        if (sourceUrl.indexOf(FSHARE_BASE + "/download/get") === 0 || data.url || data.download_url) {
            return JSON.stringify(resolveFshareWebDownloadData(data));
        }
...
function buildFshareWebFileStep(fileUrl, fileName) {
    return {
        ...
        fileName: fileName || extractFshareCode(fileUrl),
        note: "Dang nhap Fshare bang nut Dang nhap cua app truoc khi bam xem."
    };
}
...
function buildFshareWebDownloadStep(html, sourceUrl) {
    ...
    if (!body) {
        return {
            ...
            isEmbed: false,
            headers: {},
            error: "Khong lay duoc form download Fshare. Hay dang nhap Fshare bang nut Dang nhap roi thu lai."
        };
    }
...
function resolveFshareWebDownloadData(data) {
    ...
    return {
        ...
        isEmbed: false,
        headers: {},
        error: error || "Fshare chua tra link tai. Hay dang nhap Fshare, tai khoan VIP se on dinh hon tai khoan free."
    };
}
