...
function getManifest() {
    return JSON.stringify({
        ...
        "name": "Stripchat",
        "version": "1.0.7",
        "baseUrl": "https://vi.strip.chat",
        "fallbackUrls": ["https://strip.chat"],
        "loginUrl": "https://vi.strip.chat/login",
...
function getHomeSections() {
    return JSON.stringify([
        { slug: "vietnam", title: "Việt Nam", type: "Horizontal", path: "category" },
...
function getUrlList(slug, filtersJson) {
    ...
    var normalizedSlug = normalizeSlug(slug || filters.primaryTag || "girls");
    var primaryTag = normalizePrimaryTag(normalizedSlug);
    var params = {
        limit: PAGE_SIZE,
    ...
    __lastListPage = page;
    __lastPreferredBlockUrl = "";

    if (isVietnamSlug(normalizedSlug)) {
        __lastPreferredBlockUrl = "girls/vietnamese";
        params.tagId = "girls.tagLanguageVietnamese";
    }

    return buildUrl(API_MODELS_URL, params);
}
...
function parseListResponse(html) {
    try {
        ...
        var models = extractModels(data, __lastPreferredBlockUrl);
        __lastPreferredBlockUrl = "";
        var items = [];
        var seen = {};
...
function normalizeSlug(slug) {
    return String(slug || "").replace(/^\/+|\/+$/g, "").toLowerCase();
}

function isVietnamSlug(slug) {
    slug = normalizeSlug(slug);
    return slug === "vietnam" || slug === "viet-nam" || slug === "vi" || slug === "vn" || slug === "vietnamese" || slug === "girls/vietnamese";
}

function extractModels(data, preferredBlockUrl) {
    var models = [];
    var i;
    var j;
    ...
    if (preferredBlockUrl && data.blocks && data.blocks.length !== undefined) {
        for (i = 0; i < data.blocks.length; i++) {
            var preferredBlock = data.blocks[i];
            if (!preferredBlock || !preferredBlock.models) continue;

            if (preferredBlock.url === preferredBlockUrl || preferredBlock.tagId === "girls.tagLanguageVietnamese") {
                for (j = 0; j < preferredBlock.models.length; j++) models.push(preferredBlock.models[j]);
            }
        }
