...
function parseMovieDetail(html) {
    try {
        ...
        var streamName = extractText(html, /"streamName"\s*:\s*"([^"]+)"/i);
        var hlsUrl = streamName ? buildHlsUrl(streamName, "") : "";
        ...
        return JSON.stringify({
            ...
            description: "",
            servers: [{
                name: hlsUrl ? "HLS Live" : "Web Player",
                episodes: [{
                    id: hlsUrl || (username ? BASE_URL + "/" + encodeURIComponent(username) : pageUrl),
                    name: "Live",
                    slug: "live"
                }]
            }],
...
function parseDetailResponse(html, fetchedUrl) {
    try {
        ...
        if (data && data.cam) {
            ...
            var episode = detail.servers && detail.servers[0] && detail.servers[0].episodes && detail.servers[0].episodes[0];
            if (episode && episode.id && episode.id.indexOf(".m3u8") !== -1) {
                return JSON.stringify({
                    url: episode.id,
                    isEmbed: false,
            ...
            return JSON.stringify({
                url: episode ? episode.id : BASE_URL + "/",
                isEmbed: false,
                playerType: "embed",
                headers: playerHeaders(detail.id)
            });
        ...
        if (streamName) {
            return JSON.stringify({
                url: buildHlsUrl(streamName, ""),
                isEmbed: false,
                mimeType: "application/x-mpegURL",
                headers: playerHeaders(extractUsernameFromUrl(fetchedUrl || ""))
            });
        }

        return JSON.stringify({
            url: fetchedUrl || BASE_URL + "/",
            isEmbed: false,
            playerType: "embed",
    ...
    try {
        ...
        if (streamName) {
            return JSON.stringify({
                ...
                isEmbed: false,
                mimeType: "application/x-mpegURL",
                headers: playerHeaders(extractUsernameFromUrl(url || ""))
            });
        }
...
function mapCamToDetail(data) {
    ...
    var streamName = cam.streamName || "";
    var status = cleanText(user.status || cam.privateMode || "");
    var isLive = user.isLive !== false && user.isOnline !== false;
    var isPublic = status === "" || status === "public";
    var hlsUrl = (streamName && isLive && isPublic) ? buildHlsUrl(streamName, getFlashphonerServer(cam)) : "";
    ...
    return {
        ...
        servers: [{
            name: hlsUrl ? "HLS Live" : "Web Player",
            episodes: [{
                id: hlsUrl || (username ? BASE_URL + "/" + encodeURIComponent(username) : BASE_URL + "/"),
                name: "Live",
                slug: "live"
            }]
        }],
...
function getFlashphonerServer(cam) {
    if (!cam) return "";
    if (cam.viewServers && cam.viewServers["flashphoner-hls"]) return cam.viewServers["flashphoner-hls"];
    if (cam.viewServers && cam.viewServers.flashphoner_hls) return cam.viewServers.flashphoner_hls;
    return "";
}

function buildHlsUrl(streamName, server) {
    streamName = String(streamName || "").trim();
    if (!streamName) return "";

    if (server) {
        return "https://b-" + encodeURIComponent(server) + ".doppiocdn.com/hls/" + encodeURIComponent(streamName) + "/master/" + encodeURIComponent(streamName) + "_auto.m3u8?playlistType=standard";
    }

    return "https://edge-hls.doppiocdn.com/hls/" + encodeURIComponent(streamName) + "/master/" + encodeURIComponent(streamName) + "_auto.m3u8?playlistType=standard";
}
