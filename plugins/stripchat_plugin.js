...
var __lastListIsLogin = false;
var __lastDetailIsLogin = false;
var __lastPlayIsLogin = false;
...
function getHomeSections() {
    return JSON.stringify([
        { slug: "login", title: "Đăng Nhập Stripchat", type: "Horizontal", path: "category" },
...
function getUrlDetail(slug) {
    if (!slug) return "";
    slug = String(slug);
    if (slug === "__stripchat_login__") {
        __lastDetailIsLogin = true;
        return buildUrl(API_MODELS_URL, {
            limit: 1,
            offset: 0,
            primaryTag: "girls"
    ...
    if (slug === "__stripchat_login_play__") {
        __lastPlayIsLogin = true;
        return buildUrl(API_MODELS_URL, {
            limit: 1,
            offset: 0,
            primaryTag: "girls"
...
function parseListResponse(html) {
    try {
        ...
        if (__lastListIsLogin) {
            __lastListIsLogin = false;
            return JSON.stringify({
                items: [{
                    id: "__stripchat_login__",
                    title: "Đăng Nhập Stripchat",
                    ...
                    description: "Mở WebView để đăng nhập một lần. Nếu app giữ cookie, các phòng live sẽ tự nhận phiên đăng nhập.",
                    quality: "WEB",
...
function parseMovieDetail(html) {
    try {
        if (__lastDetailIsLogin) {
            __lastDetailIsLogin = false;
            return JSON.stringify(makeLoginDetail());
        }
        ...
        return JSON.stringify({
            id: username,
            title: username || "Đăng Nhập Stripchat",
            posterUrl: posterUrl,
            backdropUrl: posterUrl,
            description: username ? "" : "Đăng nhập trong WebView để lưu cookie cho các phòng live.",
            servers: [{
                name: "Web Player",
                episodes: [{
                    id: episodeUrl,
...
function parseDetailResponse(html, fetchedUrl) {
    try {
        if (__lastPlayIsLogin) {
            __lastPlayIsLogin = false;
            return JSON.stringify({
                url: BASE_URL + "/login",
                isEmbed: false,
                playerType: "embed",
...
function makeLoginDetail() {
    var iconUrl = "https://upload.wikimedia.org/wikipedia/en/thumb/f/f9/Stripchat-logo.svg/960px-Stripchat-logo.svg.png";
    ...
    return {
        ...
        posterUrl: iconUrl,
        backdropUrl: iconUrl,
        description: "Mở WebView để đăng nhập Stripchat. Sau khi đăng nhập xong, quay lại app và mở phòng live.",
        servers: [{
            name: "Web Login",
            episodes: [{
                id: "__stripchat_login_play__",
                name: "Đăng Nhập",
                slug: "login"
            }]
        }],
